...
- 状态：已处理（Windows trusted publisher 配置修复后恢复全平台发布）
...
- 已验证 `file:./<native-tarball>` 放在 root tarball manifest 中会让 Bun 在安装 root tarball 时从已解包 package 相对路径解析，导致 `ENOENT extracting tarball`。
- pack/local mode 下保留 root manifest 的 native `optionalDependencies` 版本号，同时把本次生成的 native package 目录复制到 root package 的 `node_modules/<native-package>`，并写入 `bundledDependencies`。
- root tarball 因此自带当前平台 native package；安装 root tarball 不需要访问 registry，也不需要用户同时手动安装 native tarball。普通 publish mode 不写 `bundledDependencies`，仍使用版本号 optional dependencies。
